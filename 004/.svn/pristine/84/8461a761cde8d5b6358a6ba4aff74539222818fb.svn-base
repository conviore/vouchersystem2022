 ...
 * checkPwdCaractor 校验密码 只能包含字母/数字/特殊字符
 * IsUrl 验证网址Url 
 * @author 
 *
 */
...
//		System.out.println(dataValidateUtil.chechHTML("df"));
     ...
     * 校验密码 只能包含字母/数字/特殊字符
     */
    public static boolean checkPwdCaractor(String wpd){
    	if(!checkCaractor(wpd)){
    		return false;
    	}
		Pattern pattern = Pattern.compile("(?=.*[0-9]+)(?=.*[a-zA-Z]+)(?=.*\\W+)");
		Matcher mat = pattern.matcher( wpd );
		return mat.find();
    }
    
    //只包含字母/数字/特殊字符
    public static boolean checkCaractor(String str){
